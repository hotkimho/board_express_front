  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      for (let i = 0; i < posts.data.length; i++) {
        if (posts.data[i].type === 1) posts.data[i].type = 'all';
        else if (posts.data[i].type === 2) posts.data[i].type = 'general';
        else if (posts.data[i].type === 3) posts.data[i].type = 'review';
        else if (posts.data[i].type === 4) posts.data[i].type = 'notice';
        else if (posts.data[i].type === 5) posts.data[i].type = 'music';
      }
      setPosts(posts.data);
    };
    fetchData();
  ...
  return (
    <DomesticBoardContainer>
      ...
      <Board>
        ...
        <RenderBoard posts={posts} currentPage={currentPage} currentType={currentType} />
